      ...
      {isCartOpen && (
        <>
          ...
          <motion.div 
            ...
            className="fixed top-0 right-0 h-full w-[95%] sm:w-[90%] max-w-sm bg-white z-[130] shadow-2xl flex flex-col rounded-l-[1.5rem] md:rounded-l-[2rem] overflow-hidden"
            ...
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <div className="flex flex-col">
                <h2 className="text-xl font-serif italic text-gray-900 flex items-center">
                  আপনার ব্যাগ
                  <span className="ml-2 bg-rose-500 text-white text-[10px] font-bold py-1 px-2.5 rounded-full">{toBengaliNumber(itemCount)}</span>
                </h2>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
              {cart.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center p-8">
                  <div className="w-20 h-20 bg-rose-500/5 rounded-full flex items-center justify-center mb-6">
                  ...
                  <button 
                    onClick={() => setIsCartOpen(false)}
                    className="mt-8 text-xs font-bold uppercase tracking-widest text-rose-500 border-b border-rose-500/30 pb-1 hover:border-rose-500 transition-all"
                  >
                    শপিং শুরু করুন
                  </button>
                </div>
                    ...
                    >
                      ...
                      <div className="flex-1 flex flex-col justify-between py-0.5">
                        ...
                        <div className="flex items-center justify-between mt-auto">
                          <p className="font-bold text-gray-950 text-sm">{toBengaliNumber(item.price)} ৳</p>
              ...
              <div className="p-6 border-t border-gray-100 bg-white shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
                <div className="mb-6">
                  ...
                  <div className="flex bg-gray-50 p-1 rounded-xl">
                    <button 
                      ...
                      className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${deliveryLocation === 'insideDhaka' ? 'bg-white shadow-sm text-gray-900 font-black' : 'text-gray-500 hover:text-gray-900'}`}
                    >
                      ঢাকার ভেতরে
                    </button>
                    <button 
                      onClick={() => setDeliveryLocation('outsideDhaka')}
                      className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${deliveryLocation === 'outsideDhaka' ? 'bg-white shadow-sm text-gray-900 font-black' : 'text-gray-500 hover:text-gray-900'}`}
                    >
                      ঢাকার বাইরে
                    </button>
                  </div>
                ...
                <div className="flex justify-between items-center mb-6 pt-4 border-t border-gray-100">
                  <div className="flex flex-col">
                    ...
                    <span className="text-3xl font-serif text-gray-900 font-medium leading-none">
                ...
                <a 
                  href="#/order-form" 
                  onClick={() => setIsCartOpen(false)}
                  className="w-full bg-rose-500 hover:bg-rose-600 text-white font-black py-4 px-6 rounded-[13px] flex items-center justify-center transition-all shadow-xl shadow-rose-500/20 active:scale-[0.98] uppercase tracking-widest text-xs"
                >
                  চেকআউট করুন
                </a>
                <button 
                  onClick={() => setIsCartOpen(false)}
                  className="w-full text-center mt-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest hover:text-rose-500 transition-colors"
                >
                  শপিং চালিয়ে যান
                </button>
              </div>
